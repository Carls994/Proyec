...
export default function AdminPage() {
  ...
  // Cálculo del total acumulado y disponible en caja
  ...
  // Registrar un nuevo gasto con validación de saldo disponible
      ...
      <div className="w-full max-w-2xl space-y-6">
        ...
        <div className="flex justify-between items-center border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400">Control de Caja y Gastos Destinados</p>
          </div>
          <Link
            href="/"
        ...
        {/* Resumen de Caja en Admin */}
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
            <span className="text-[10px] font-bold text-slate-400 uppercase block">Ingresos Totales</span>
          ...
          <div className="p-3 rounded-xl bg-cyan-950/40 border border-cyan-500/40">
            <span className="text-[10px] font-bold text-cyan-400 uppercase block">Disponible en Caja</span>
        ...
        <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-3">
          <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
            💸 Registrar Nuevo Egrezo / Gasto de Caja
          </h2>

          <form onSubmit={handleRegistrarGasto} className="space-y-3">
            <div>
              <label className="block text-xs text-slate-400 mb-1">Concepto o Destino del Dinero</label>
              <input
                type="text"
                placeholder="Ej: Entrega de seña para el local"
                value={conceptoGasto}
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Monto a Destinar (Gs.)</label>
              <input
                type="number"
            ...
            >
              {submittingGasto ? 'Guardando...' : 'Confirmar y Registrar Gasto'}
            </button>
          </form>
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-3">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {gastos.length === 0 ? (
              <p className="text-xs text-slate-500 italic text-center py-4">No hay gastos registrados aún.</p>
            ) : (
              gastos.map((g) => (
                <div key={g.id} className="p-3 rounded-lg bg-slate-950 border border-slate-800/80 flex justify-between items-center text-xs">
                  <div>
